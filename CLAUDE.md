...
| Prompt (Generate mode) | `/prompt?mode=generate` | route `prompt.tsx` + `src/components/generate/GenerateWorkspace.tsx`; behind the native-generation feature flag (`src/lib/generation/feature-flag.ts`), falls back to Build mode when off; `/generate` 301s here |
| Prompt (Build mode) | `/prompt?mode=build` | route `prompt.tsx` + `src/components/prompt/BuildMode.tsx`, API `/api/public/generate-prompt`, history `kind: "generate"` |
| Prompt (Critique mode) | `/prompt?mode=critique` | route `prompt.tsx` + `src/components/prompt/CritiqueMode.tsx`, API `/api/public/critique-prompt`, mode `CRITIQUE`, history `kind: "critique"`; `/critique` 301s here |
| Gallery | `/gallery` | "Use as reference" → `/prompt?mode=build&ref=/gallery/<file>` |
| Templates | `/templates` | footer only; `src/data/templates.ts` is the one source for the page, MCP, and `/prompt?mode=build&template=<slug>` |
| Blog | `/blog` | "Field Notes" in product copy |
| MCP | `/integrations/mcp` (page), `/mcp` (endpoint) | footer + homepage; six read-only tools in `src/lib/mcp/`: search_prompts/get_prompt, list_templates/get_template, search_guides/get_guide |

- Generate, Build, and Critique are three modes of one workspace (`/prompt`), not separate products: the segmented control says "Generate", "Build", and "Critique". Header nav has one entry for this workspace, inserted by `Header.tsx` (not part of the frozen `NAV_ITEMS` export) between Library and Gallery — labeled "Generate" and linking to `/prompt?mode=generate` when the native-generation feature flag is on, or "Prompt" linking to `/prompt` when it's off (Build/Critique predate the flag and must stay reachable without it). Header is Library · Generate · Gallery once the flag is on. Blog, Templates, and MCP live in the footer. Homepage product story: Library, Prompt, Gallery, Templates, MCP, Field Notes. All three modes stay mounted so each keeps its own draft. In-page copy still uses "Build Prompt" / "Critique Prompt". Never "generator" or "image generator" in current product UI or metadata (native generation itself is a real, shipped feature — "Generate image" is the accurate CTA there). All shared labels, SEO strings, and JSON-LD names live in `src/lib/product.ts`; tests read the same source.
...
- Labels: Library · Generate · Gallery (`NAV_ITEMS` in `src/lib/product.ts` holds only Library/Gallery — `Header.tsx` inserts the unified workspace entry between them, see above). Blog, Templates, and MCP live in the footer only.
