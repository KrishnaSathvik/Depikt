...
- Builder and Critic are two modes of one workspace: nav says "Prompt", the segmented control says "Build" and "Critique". Both modes stay mounted so each keeps its own draft. In-page copy still uses "Build Prompt" / "Critique Prompt". Never "generator", "Generate prompt", or "image generator" in current product UI or metadata. All shared labels, SEO strings, and JSON-LD names live in `src/lib/product.ts`; tests read the same source.
