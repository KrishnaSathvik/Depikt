...
| Prompt (Build mode) | `/prompt?mode=build` | route `prompt.tsx` + `src/components/prompt/BuildMode.tsx`, API `/api/public/generate-prompt`, history `kind: "generate"`; `/generate` 301s here |
...
- Build and Critique are two modes of one workspace, not separate products: nav says "Prompt", the segmented control says "Build" and "Critique". Header is Library · Prompt · Gallery; Blog, Templates, and MCP live in the footer. Homepage product story: Library, Prompt, Gallery, Templates, MCP, Field Notes. Both modes stay mounted so each keeps its own draft. In-page copy still uses "Build Prompt" / "Critique Prompt". Never "generator", "Generate prompt", or "image generator" in current product UI or metadata. All shared labels, SEO strings, and JSON-LD names live in `src/lib/product.ts`; tests read the same source.
...
- Labels: Library · Prompt · Gallery (from `NAV_ITEMS` in `src/lib/product.ts`). Blog, Templates, and MCP live in the footer only.
