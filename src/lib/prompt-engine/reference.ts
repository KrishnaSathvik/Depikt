...
Open with an explicit instruction such as: "Use the attached sketch as the layout guide: follow its regions, positions, relative sizes, and reading order exactly, rendered at production quality." Then describe every region and its position (top/left/right/bottom, relative widths and heights), the hierarchy, and what each labeled placeholder represents. Do not inherit the rough line quality, hand-drawn look, or placeholder wording unless the user asks for it. Where the sketch marks text regions and the user supplied no copy, describe each region's role (headline, subline, button label, caption) and keep it brief instead of inventing sample sentences; never instruct the model to include "example copy". Reserve bracketed placeholders for facts the user must supply (dates, prices, names). Use the user's own words (names, product, headline) wherever they gave them. Close with "text appears only in the regions described, nothing else" rather than "no other text", so the text rule cannot contradict the regions you asked for.`;
    case "none":
    default:
      return "";
  }
