...
test("temperature is sent only when the model supports it", () => {
  assert.deepEqual(
    resolveRequestParams({ model: "gpt-5.4-mini", temperature: 0.7, timeoutMs: 1 }),
    { temperature: 0.7 },
  );
...
test("reasoning effort is validated per model", () => {
  assert.deepEqual(
    resolveRequestParams({ model: "gpt-5.6-luna", reasoningEffort: "none", timeoutMs: 1 }),
    {
...
test("cost estimate uses cached-input pricing and counts output tokens", () => {
  const usd = estimateCostUsd("gpt-5.6-terra", {
